frappe.ui.form.on("Delivery Note", {
    setup(frm) {
        ...
        frm.set_query("wh_cell", "items", (_, cdt, cdn) => {
            const row = locals[cdt][cdn];
            return {
                query: "pharma_vn.api.warehouse_layout.search_pickable_cells",
                filters: {
                    warehouse: row.warehouse,
                    layout: row.wh_layout,
                    item_code: row.item_code,
                    required_qty: row.stock_qty || row.qty || 0,
    ...
    refresh(frm) {
        ...
        refreshPickingFieldIndicators(frm);
    },

    async before_submit(frm) {
        const pendingRows = await getPendingOutboundRows(frm);
        applyPickingFieldIndicators(frm, pendingRows);
        if (!pendingRows.length) {
            return;
        }

        frappe.validated = false;
        frappe.throw(__("Thiếu vị trí xuất kho. Vui lòng chọn đủ WH Layout và WH Cell cho các dòng hàng dùng Warehouse Layout 2D."));
    },
});
...
frappe.ui.form.on("Delivery Note Item", {
    ...
    wh_cell(frm, cdt, cdn) {
        ...
        frappe.db
            .get_value("WH Cell", row.wh_cell, ["layout", "warehouse"])
            .then((response) => {
                const data = response.message || {};
                if (data.layout && data.layout !== row.wh_layout) {
...
async function getPendingOutboundRows(frm) {
...
function applyPickingFieldIndicators(frm, pendingRows) {
...
async function refreshPickingFieldIndicators(frm) {
    const pendingRows = await getPendingOutboundRows(frm);
    applyPickingFieldIndicators(frm, pendingRows);
}
